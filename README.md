...
3. [Graphiti Framework](#3-graphiti-framework)
    *   [3.1 What is Graphiti?](#31-what-is-graphiti)
    *   [3.2 The Core Innovation — Temporal Awareness](#32-the-core-innovation--temporal-awareness)
...
## 3. Graphiti Framework

### 3.1 What is Graphiti?

**Graphiti** is an open-source framework for building knowledge graphs that supports real-time, temporal data. It provides a structured approach for managing context, which is particularly relevant for LLM-based agents and advanced RAG pipelines.

Graphiti was created by **Zep AI** to solve a specific problem: traditional RAG systems fail when data changes frequently, because they treat knowledge as a static library of disconnected chunks. Graphiti represents a meaningful departure from traditional RAG methods, specifically because it was built from the ground up as a memory infrastructure for dynamic agentic systems.

---

### 3.2 The Core Innovation — Temporal Awareness

Most knowledge graph systems store the **current state** of the world. Graphiti stores the **history of the world**.

Every edge (relationship) in Graphiti includes:
...
Edge = {
    ...
    "relation":   "works_at",
    "object":     "TechCorp",
    "t_valid":    "2023-01-01",   # when this fact BECAME true
    "t_invalid":  "2024-06-15",   # when this fact STOPPED being true
    "created_at": "2023-01-02",   # when it was added to the graph
}
```

This **bi-temporal model** means:
- You can query "What was true on March 15, 2023?"
- You can see how knowledge evolved over time
- Contradictions are handled by invalidating old edges, not deleting them
...
Traditional RAG: Retrieves all three, cannot determine CURRENT plan.
Graphiti:        Maintains timeline of plan changes, answers:
                 "Customer is currently on Pro Plan."  ✅
```

---

### 3.3 Graphiti Architecture
...
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                   INPUT LAYER                           │    │
│  │  Unstructured Text │ JSON Data │ Chat Messages          │    │
│  └──────────────────────────┬──────────────────────────────┘    │
│                             │                                    │
│                             ▼                                    │
│  ┌─────────────────────────────────────────────────────────┐    │
...
│  │                                                         │    │
│  │  1. LLM Entity Extraction  → Nodes                     │    │
│  │  2. LLM Relation Extraction → Edges                    │    │
│  │  3. Conflict Resolution    → Invalidate old edges      │    │
│  │  4. Embedding Generation   → Semantic search index     │    │
...
│  │                                                         │    │
│  │   Neo4j / FalkorDB / Kuzu / AWS Neptune                │    │
│  │                                                         │    │
│  │   Entity Nodes ──[RELATES_TO edges]──▶ Entity Nodes    │    │
│  │        │                                    │          │    │
│  │        └──── Episode Nodes (source text) ───┘          │    │
│  └──────────────────────────┬──────────────────────────────┘    │
│                             │                                    │
│                             ▼                                    │
│  ┌─────────────────────────────────────────────────────────┐    │
...
│                             │                                    │
│                             ▼                                    │
│              Ranked, temporally-filtered facts                   │
│                     (for LLM context)                            │
└──────────────────────────────────────────────────────────────────┘
```
...
### 3.4 Key Graphiti Concepts

#### Episodes
An **Episode** is the fundamental unit of information ingestion — a discrete event, conversation, or document that gets processed into the graph.

```python
# Three types of episodes
EpisodeType.text     # plain text, documents, notes
EpisodeType.message  # chat messages with role (user/assistant)
EpisodeType.json     # structured data (CRM records, logs, etc.)
```

#### Entity Nodes
Automatically extracted from episodes by an LLM. Each node has:
- A `name` and `summary`
- Vector embeddings for semantic search
- A `group_id` for multi-tenancy

#### Entity Edges (Facts)
The relationships between nodes. Each edge has:
- `fact` — a human-readable string of the relationship
- `t_valid` / `t_invalid` — temporal validity window
- `episodes` — which episodes this fact came from

#### Episodic Edges
Links between Episodes and the Entities they mention — the "source provenance" of every fact.

---

### 3.5 Graphiti's Hybrid Search

Graphiti's hybrid indexing system — combining semantic embeddings, keyword search, and graph traversal — allows rapid retrieval in near-constant time, independent of graph scale.

```
Query: "What is Alice currently working on?"
    ↓
┌─────────────────────────────────────────────┐
│  1. SEMANTIC SEARCH                         │
│     Query embedding vs. edge fact embeddings│
│     → Finds: "Alice works on ProjectX"      │
│              "Alice assigned to feature Y"  │
├─────────────────────────────────────────────┤
│  2. BM25 KEYWORD SEARCH                     │
│     Exact/fuzzy keyword matching            │
│     → Finds: edges mentioning "Alice"       │
├─────────────────────────────────────────────┤
│  3. GRAPH TRAVERSAL                         │
│     Walk from Alice node through edges      │
│     → Finds: connected projects, teams      │
├─────────────────────────────────────────────┤
│  4. CROSS-ENCODER RERANKING                 │
│     Re-score all results for final ranking  │
└─────────────────────────────────────────────┘
    ↓
Top-K temporally valid facts
```

---

### 3.6 Graphiti vs Microsoft GraphRAG

Both are Graph RAG frameworks but with different philosophies:

| Aspect | Microsoft GraphRAG | Graphiti |
|---|---|---|
| **Primary goal** | Query-focused summarization | Real-time agent memory |
| **Update model** | Batch — full recomputation | Incremental — instant updates |
| **Data type** | Static document corpora | Dynamic streams (chat, JSON, docs) |
| **Temporal model** | None | Full bi-temporal tracking |
| **LLM usage at query time** | Heavy — multi-LLM calls for summaries | Light — retrieval, then LLM once |
| **Latency** | Higher (summary generation) | Lower (pre-computed graph) |
| **Best for** | Document corpora analysis | Live agents, CRM, conversational AI |

---

### 3.7 Supported Backends

```
Graph Databases:   Neo4j | FalkorDB | Kuzu | AWS Neptune
LLM Providers:     OpenAI | Anthropic | Google Gemini | Groq | Ollama (local)
Embedders:         OpenAI | Voyage AI | Sentence Transformers | Gemini
Rerankers:         Cross-encoder (via sentence-transformers)
Integrations:      LangChain | LangGraph | MCP (Claude, Cursor)
```
